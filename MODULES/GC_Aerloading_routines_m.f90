MODULE GC_Aerloading_routines_m
! This tool is taken from Rob's (as in GEMS-tool) Added September 2017
! Three profile types, boxcar, exponential and generalized distribution function (GDF)
...
subroutine generate_plume &
     ...
     d_profile_drlx, fail, message)              ! output
...
! z_upperlimit = uppermost height value at start  of profile
! z_lowerlimit = lowest    height value at bottom of profile
...
! Exponential relaxation for EXP
...
!   Ashplume:  (w.r.t. total tau and peak height and half width)
  ...
  REAL(KIND=8), DIMENSION(MAX_LAYERS), INTENT(OUT) :: d_profile_drlx
...
! Initialize
  do i = 1, nlayers
     ...
     d_profile_drlx(i) = 0.0d0
  enddo
  
! Call
  do_derivatives = .true.
  IF (profile_type == 'BOX') THEN
     call profiles_uniform ( &
          max_layers, nlayers, level_heights, do_derivatives, & ! Inputs
          z_upperlimit, z_lowerlimit, total_column, & ! Inputs
          profile, d_profile_dcol, & ! output
          fail, message, action )
  ELSE IF (profile_type == 'EXP') THEN
     call profiles_expone ( &
          max_layers, nlayers, level_heights, do_derivatives, & ! Inputs
          z_upperlimit, z_lowerlimit, relaxation, total_column,  & ! Inputs
          profile, d_profile_dcol, d_profile_drlx, & ! Output
          fail, message, action )
  ELSE IF (profile_type == 'GDF') THEN
  ...
  ELSE
     ! We should be never be here
     print*, 'NO AEROSOL PROFILE TYPE MATCHING INPUT '//profile_type
     print*, 'Please check aerosol input configuration'
     STOP
  END IF
  RETURN
...
End Module GC_Aerloading_routines_m
