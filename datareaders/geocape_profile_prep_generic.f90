...
! 5. If the data is specified for layer, then Pressure, altitude, TATM is for top of the layer
!    Gas is for layer mean VMR
! 6. Aerosol and cloud data can only be specified at each layer
    
!TES_File_ID = "L2: True Full State Vector Data"
!Data_Size = 48 x 10                        ***
!surfaceTemperature = 300.159               ***
!PSUR = 975.433                             ***
!ProfileIsLevel = 1                         ***                   
!latitude = 42.7                            ***
!longitude = -74.1                          ***
!End_of_Header  ****  End_of_Header  ****  End_of_Header  ****  End_of_Header
!Level Pressure    Altitude     TATM     H2O       O3        NO2        HCHO       SO2        CO   ***
!-     hPa         m            K        VMR       VMR       VMR        VMR        VMR        VMR
!
!

! The file format is based on TES profile used in GEOCAPE sensitivity studies
...
subroutine geocape_profile_reader_1 &
           ( filename, maxl, maxc,                                                          &     ! input
             profile_data, footprint_data, profids, profile_is_level, GC_nlayers, message, fail ) ! output
...
!  Main output
   ...
   real(kind=8), dimension(4),     intent(out)     :: footprint_data
...
!  Local variables
   ...
   real(kind=8) :: surfTemp, psurf, lat, lon
   character*80 :: dummy
