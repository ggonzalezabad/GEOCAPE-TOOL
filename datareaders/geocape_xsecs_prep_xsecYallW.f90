...
!  Local variables
   ...
   integer       :: nbuff, ndata, n, g, np, nf, ngas_check, maxdata, errstat
...
!  ===
      ...
      if ( which_gases(g) .eq. 'NO2 ' .and. gas_xsecs_type(g) .eq. 1) then
...
!  ==
      ...
      else if ( which_gases(g) .eq. 'O3  ' .and. gas_xsecs_type(g) .eq. 2) then
...
!  Spline the data, no negative values

         call spline(x,y,nbuff,0.0d0,0.0d0,y2)
         do n = 1, nlambdas
           call splint(x,y,y2,nbuff,lambdas(n),xsec)
           if ( xsec .gt. 0.0d0 )  gas_xsecs(n,1,g) = xsec / conversion
         enddo
...
!  ===
      ...
      else if ( which_gases(g) .eq. 'H2O ' .and. gas_xsecs_type(g) .eq. 1) then
...
!  .. Read first line, and check window is covered
         ...
         if (lamb1 .lt. wav ) then
            fail = .true.
            message = 'H2O Xsec data does not cover input window at lower end'
            go to 91
         endif
...
!  .. Check if last data line is present, then window not covered
         if (ndata .eq. maxdata) then
            fail = .true.
            message = 'H2O Xsec data does not cover input window at upper end'
            go to 91
         endif
         close(1)         
...
!  .. Read first line, and check window is covered
         ...
         if (lamb1 .lt. wav ) then
            fail = .true.
            message = 'O4 Xsec data does not cover input window at lower end'
            go to 91
         endif
!  .. Read more lines, saving to spline buffer
         reading = .true.
...
!  .. Check if last data line is present, then window not covered
         if (ndata .eq. maxdata) then
            fail = .true.
            message = 'O4 Xsec data does not cover input window at upper end'
            go to 91
         endif
         close(1)
...
!  ===
      ...
      else if ( which_gases(g) .eq. 'O2  ' .and. gas_xsecs_type(g) .eq. 1) then
...
!  .. Read first line, and check window is covered
         ...
         if (lamb1 .lt. wav ) then
            fail = .true.
            message = 'O2 Xsec data does not cover input window at lower end'
            go to 91
         endif
!  .. Read more lines, saving to spline buffer
         reading = .true.
...
!  .. Check if last data line is present, then window not covered
         if (ndata .eq. maxdata) then
            fail = .true.
            message = 'O2 Xsec data does not cover input window at upper end'
            go to 91
         endif
         close(1)
