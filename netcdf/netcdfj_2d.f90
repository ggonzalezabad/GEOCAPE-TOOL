      SUBROUTINE NETCDFJ_2D(NWAV,NLAY,WAV,HGT,TEMP,DUMMY,DLEN,FNAME, &
			   FLEN,UNITNAME,UNITLEN,LAT,LON,DATE,MON,YEAR, &
			   NSZA,SZA,NVZA,VZA,NPHI,PHI)
...
! The number of dimensions we will have: Wav, Lay
...
! allocation.f
      ...
      DOUBLE PRECISION, DIMENSION(NWAV,NLAY), INTENT(IN) :: TEMP
      DOUBLE PRECISION, DIMENSION(NWAV), INTENT(IN) :: WAV

      DOUBLE PRECISION, DIMENSION(NLAY)             :: MHGT
...
! going for a length of one data slice
      ...
      MHGT(1:NLAY) = (HGT(0:NLAY-1) + HGT(1:NLAY))/2.0
!===================***********************************=======================
...
!=============================================================================
        ...
        LAYID       = NCVID (NCID, 'LayerHeight', RCODE)
...
! Create the dimensions of the dataset:
        ...
        LAYDIM  = NCDDEF (NCID,  'LayerHeight', NLAY,  RCODE)
...
!=============================================================================
        ...
        WAVID  = NCVDEF (NCID, 'Wavelength', NCDOUBLE, 1, WAVDIM,  &
                         RCODE)
        LAYID  = NCVDEF (NCID, 'LayerHeight', NCDOUBLE, 1, LAYDIM,  &
     		         RCODE)
...
! Trace Gas/Temperature Jacobian:
...
!=============================================================================
        ...
        CALL NCAPTC (NCID, HGTID, 'units', NCCHAR, 2, 'km', &
                     RCODE)
        CALL NCAPTC (NCID, LAYID, 'units', NCCHAR, 2, 'km', &
                     RCODE)

        CALL NCAPTC (NCID, TEMPID, 'units', NCCHAR, UNITLEN, TRIM(UNITNAME), &
                          RCODE)
        MINMAX = (/MINVAL(TEMP), MAXVAL(TEMP)/)
        CALL NCAPT(NCID, LAYID, 'valid_range', NCLONG, 2, MINMAX, RCODE)
...
!=============================================================================
        ...
        CALL NCVPT (NCID, LAYID, 1, NLAY, MHGT, RCODE)
