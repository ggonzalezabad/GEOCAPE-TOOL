...
C  This is the Lambertian or BRDF surface routine
C     Reflected homogeneous solutions

C  merged July 26, 2010. Using new BRDF material
C   RT SOLUTIONS Inc,
...
C  Return if Fourier component not zero for Lambertian surface

      IF ( DO_LAMBERTIAN_SURFACE 
     &     .and. FOURIER_COMPONENT .GT. 0 ) RETURN

C  Go to total BRDF case for non-Lmabertian surface
...
C  Return if not the Fourier m = 0 component (Lambertian surface)

      IF ( DO_LAMBERTIAN_SURFACE .and.
     &       FOURIER_COMPONENT .GT. 0 ) RETURN

C  Skip if non-Lambertian surface
...
C    Only required for the Specialise Option 2 case
      ...
      IF ( DO_SPECIALIST_OPTION_2 ) THEN
        IF ( ACTIVE_LAYERS(NLAYERS_TEL).EQ.NLAYERS ) THEN
          CALL BVP_SURFACE_SETUP_BEAM
     I       ( DO_INCLUDE_SURFACE,
     I         FOURIER_COMPONENT, ! IBEAM, by xliu
     I         SURFACE_FACTOR )
        ENDIF
      ENDIF
