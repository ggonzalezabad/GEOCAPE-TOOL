...
C  local variables
      ...
      INTEGER          I, IDIR, UT, UA, W, O, N ! xliu, add N
...
C  TOA contribution functions (only if flagged)
...
C    STOKES. Add the single scatter component if flagged
...
C    CONTRIBS. Add the single scatter component if flagged
C     If set, now looking at convergence on RADIANCE = DIFFUSE + SSEXACT
C     Version 2.1.   Added outgoing correction flag to this.....
C     Version 2.3    Added Full single scatter flag
...
C    STOKES. Add the direct beam component if flagged (upwelling only)
...
C  TOA_CONTRIBS: For each azimuth, add Fourier component
