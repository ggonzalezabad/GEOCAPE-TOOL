...
C #                 outgoing_sphergeom_fine_up                  #
C #                 outgoing_sphergeom_fine_dn                  #
C #                 outgoing_integration_up                     #
C #                 outgoing_integration_dn                     #
C #                 multi_outgoing_adjustgeom                   #
C #                                                             #
C #      Version 2.3 outgoing DB correction                     #
C #                                                             #
C #            VLIDORT_DBCORRECTION                             #
C #                                                             #
...
C #            Correctly done, October 2010.                    #
C #                                                             #
C #            Correction for Downwelling SSCORR_OUTGOING       #
C #            Separate Geometry routine implemented.           #
C #            Implemented by R. Spurr, 10/06/10                #
C #                                                             #
C ###############################################################
...
C    Addition of Single scatter Contribution functions
...
C  Greek matrix components (local usage)
...
c        DO N = 1, NLAYERS
c          FMAT(V,N,INDEX_22) = FMAT(V,N,INDEX_11)
c          FMAT(V,N,INDEX_33) = FMAT(V,N,INDEX_44)
c        END DO
c      END DO
...
C  Call to scattering law for phase matrix ZMAT
     ...
     I          ( DO_SSCORR_TRUNCATION, SUNLIGHT, N, NSTOKES,
     I            NGREEK_MOMENTS_INPUT, LAYER_MAXMOMENTS(N),
     I            GREEKMAT_TOTAL_INPUT, SSFDEL(N),
...
C
     ...
     I  ( DO_SSCORR_TRUNCATION, DO_SUNLIGHT, LAYER, NSTOKES,
     I    NGREEKMOMS,  LAYER_MAXMOMENTS, GREEKMATRIX, TRUNCFACTOR,
     I    CTHETA, STHETA, CALPHA, SALPHA, CPHI, PHI, COSSCAT, VSIGN,
     O    ZMAT, FMAT )
...
C  Control for using Natural light case
...
C    Proper usage, 01 October 2010. R. Spurr
...
!     We were using Degrees for the S1/S2 Sign reversal
...
c
     ...
     o    partials_fineidx, lospaths, lospaths_p_up,
     o    theta_all, phi_all, cosscat_up,
     o    fail, message )

C  Completely stand-alone geometry routine for the outgoing correction
C   This is applicable to the Upwelling path geoemtry
...
C  Outgoing sphericity geometry
C  ============================
...
c
     ...
     o    sunpaths_fine, radii_fine, ntraverse_fine, alpha_fine,
     o    sunpaths_p,    radii_p,    ntraverse_p,    alpha_p,
     o    partials_fineidx, lospaths, lospaths_p_dn,
     o    theta_all, phi_all, cosscat_dn,
     o    fail, message )
...
C  Completely stand-alone geometry routine for the outgoing correction
C   This is applicable to the Downwelling path geoemtry
...
C  define Unit solar vector at BOA
C   Note change of sign for ex here.
...
C  Prepares Exact Direct Beam reflection
...
C  assign reflection term
C  initialize cumulative source term = REFLEC * F.mu_0.T/pi
C    T = Attenuation of direct beam to BOA, F = Flux
...
C  Initialize cumulative count
C    Multiply by Flux multiplier
