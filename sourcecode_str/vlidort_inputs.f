...
C  TOA contributions
C    TOA Upwelling must be set, if you are using this flag

      IF ( DO_TOA_CONTRIBS ) THEN
        IF ( .not. DO_UPWELLING ) THEN
          MAIL   = 'Bad input TOA contributions, Upwelling Not set'
          ACTION = 'Must set the DO_UPWELLING flag'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, ACTION, STATUS )
        ENDIF
      ENDIF
      ...
      IF ( DO_TOA_CONTRIBS ) THEN
        ...
        IF ( N .EQ. N_USER_LEVELS ) THEN
          MAIL   = 'Bad input TOA contributions, Level TOA not set'
          ACTION = 'Must set one level output to be TOA (0.0)'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, ACTION, STATUS )
        ENDIF
      ENDIF
