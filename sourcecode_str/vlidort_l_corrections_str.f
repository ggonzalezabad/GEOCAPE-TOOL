...
C #                                                             #
C #   3. DB correction, Lambertian and BRDF surfaces            #
C #      Version 2.4.  LAC module (column Jacobians)            #
C #                                                             #
C #            VLIDORT_LAP_DBCORRECTION                         #
C #            VLIDORT_LAC_DBCORRECTION                         #
C #            VLIDORT_LS_DBCORRECTION                          #
C #                                                             #
...
C  Get L_Z matrices for each layer, if the scatter law is linearized
     ...
     I  ( DO_SUNLIGHT, NSTOKES, NGREEK_MOMENTS_INPUT, NV, NV_PARAMETERS,
     I    N_GEOMETRIES, NBEAMS, N_USER_STREAMS, N_USER_RELAZMS,
     I    DO_SCATMAT_VARIATION, LAYER_MAXMOMENTS,
     I    GREEKMAT_TOTAL_INPUT, L_GREEKMAT_TOTAL_INPUT,   
     I    DO_SSCORR_TRUNCATION, SSFDEL, L_SSFDEL, VIEWSIGN, VZA_OFFSETS,
...
C  Include files linearized input variables
...
C
     ...
     I  ( DO_SUNLIGHT, NSTOKES, NGREEKMOMS, NV, NV_PARAMETERS,
     I    N_GEOMETRIES, NBEAMS, N_USER_STREAMS, N_USER_RELAZMS,
     I    DO_SCATMAT_VARIATION, LAYER_MAXMOMENTS, GREEKMAT, L_GREEKMAT,
     I    DO_SSCORR_TRUNCATION, SSFDEL, L_SSFDEL, VSIGN, VZA_OFFSETS,
...
C  control
      ...
      LOGICAL          DO_SUNLIGHT
      INTEGER          NSTOKES, NGREEKMOMS, NV, NV_PARAMETERS
...
C  scattering input information
      ...
      DOUBLE PRECISION GREEKMAT
     &      ( 0:MAXMOMENTS_INPUT, MAXLAYERS, MAXSTOKES_SQ )
      DOUBLE PRECISION L_GREEKMAT
...
C    P2P2 and P2M2 added 20 March 2006
...
C  Set the local Greek matrix elements that you need
c   44 and 34 are not required with natural sunlight (default here)
c   22 and 33 required for non-Mie spheroidal particles
...
c  Non-sunlight cases. (4,4) and (3,4)
...
c  Non-sunlight cases. (2,2) and (3,3)
...
C   Additional functions P2M2 and P2P2 zero for m = 0
...
C  introduce the P2P2 and P2M2 functions for L = 2
...
C  This section adds F-matrix terms needed for randomly-oriented spheroids
...
c   This must be done AFTER the moment loop.
...
C  This code is no longer required, as we have introduced code now
C   for randomly oriented spheroids. The symmetry should still of
C   course be present for the Mie particles, so this will be a
C   check on the new code.
C   R. Spurr and V. Natraj, 20 March 2006
...
C  Create Linearized Z matrices
C  ----------------------------
...
C    ** only need the first Column of the Z-matrix
...
C  For polarized case, Non-sunlight general case
...
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
C  Extension and Changes 01-05 October 2010, R. Spurr
C   - Corrections for the PHI > 180 case
C   - Addition of non-Mie (spheroidal particle) entries to F-matrix
C   - Generalization of Z-matrix to Non-sunlight situations.
C@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
C  Cumulative transmittances
C    New, 27 January 2010. TOA Contribution functions Code.
...
C  Call to geometry   
     ...
     o    PARTLAYERS_layerfineidx, lospaths, lospaths_ut_up,
     o    theta_all, phi_all, cosscat_up,
     o    fail, message )
...
!  Exception handling
              ...
              if ( fail ) then
                write(CV,'(I3)')V
                message = 'Error from outgoing_sphergeom_fine_up, '//
     &                  ' Geometry '//CV
                return
              endif
...
C  Get the scattering matrix and its linearization
     ...
     I      ( DO_SSCORR_TRUNCATION, SUNLIGHT, DO_ATMOS_LINEARIZATION,
     I        N, NSTOKES, NGREEK_MOMENTS_INPUT,   
     I        LAYER_VARY_FLAG(N), LAYER_VARY_NUMBER(N),
     I        LAYER_MAXMOMENTS(N),  GREEKMAT_TOTAL_INPUT,   SSFDEL,
     I        DO_SCATMAT_VARIATION, L_GREEKMAT_TOTAL_INPUT, L_SSFDEL,
     I        CTHETA, STHETA, CALPHA, SALPHA, CPHI,
     I        PHI_ALL(N), CSA, VSIGN,
     O        ZMAT_LOCAL, L_ZMAT_LOCAL, FMAT_LOCAL )

C  Phase matrix + linearized (multiplied by TMS factor). Save them.
C     Sunlight only, the first column of the matrix
C    General case, code programmed 05 October 2010
...
C  Call to geometry
     ...
     o    sunpaths_fine, radii_fine, ntraverse_fine, alpha_fine,
     o    sunpaths_ut,   radii_ut,   ntraverse_ut,   alpha_ut,
     o    PARTLAYERS_layerfineidx, lospaths, lospaths_ut_dn,
     o    theta_all, phi_all, cosscat_dn,   
     o    fail, message )
...
!  Exception handling
              ...
              if ( fail ) then
                write(CV,'(I3)')V
                message = 'Error from outgoing_sphergeom_fine_dn, '//
     &                  ' Geometry '//CV
                return
              endif
...
c                if ( n.gt.23) write(*,*)v,n,L_ZMAT_DN(1,V,N,1,1)
...
C  Cumulative tranmsittances (TOA contribution functions)
C     New section, 27 Janaury 2010
...
C      Programmed for General case (including Sunlight)
...
C
     ...
     I  ( DO_SSCORR_TRUNCATION, DO_SUNLIGHT, DO_LINEAR,
     I    LAYER, NSTOKES, NGREEKMOMS,
     I    LAYER_VARY_FLAG, LAYER_VARY_NUMBER,
     I    LAYER_MAXMOMENTS, GREEKMATRIX,   SSFDEL,
     I    DO_SCATMAT, L_GREEKMATRIX, L_SSFDEL,
     I    CTHETA, STHETA, CALPHA, SALPHA, CPHI,
     I    PHI, COSSCAT, VSIGN,
     O    ZMAT, L_ZMAT, FMAT )
...
C  control integers and sunlight flag
...
C  Z-Matrix, first column only. F-matrix ( sunlight case )
C  Generalized, 05 October 2010, non-tested

      DOUBLE PRECISION    ZMAT ( 4, 4 )
      DOUBLE PRECISION  L_ZMAT ( 4, 4, MAX_ATMOSWFS )
...
C  Generalized spherical functions
C    P2P2 and P2M2 added, 05 October 2010
...
c      if ( v.eq.42.and.n.eq.1.and.vsign.gt.0.0d0 ) then
c         write(*,*)'jjj'
c         write(*,*)CTHETA, STHETA, CALPHA, SALPHA, CPHI,
c     I    PHI, COSSCAT, VSIGN
c      endif
...
C  F-matrices and Linearized F-matrices
C  ------------------------------------
...
c   This must be done AFTER the moment loop. (non-sunlight only)
...
C   for randomly oriented spheroids. The symmetry should still of
C   course be present for the Mie particles, so this will be a
C   check on the new code. R. Spurr and V. Natraj,, 20 March 2006
        
c      IF ( .not. DO_SUNLIGHT ) THEN
c        FMAT(INDEX_22) = FMAT(INDEX_11)
c        FMAT(INDEX_33) = FMAT(INDEX_44)
c        IF ( DO_LINEAR ) THEN
c          IF ( LAYER_VARY_FLAG ) THEN
c            DO Q = 1, LAYER_VARY_NUMBER
c              IF ( DO_SCATMAT(N,Q) ) THEN
...
C  Z-matrix calculation
C  ====================
         
C  Initialize Z-matrix + linearization
...
C  Natural Light, first column only
...
C  Other sources, use full 4x4 matrix
C    Code introduced but not tested, 05 October 2010
...
C  Zero the attenuation factors.  EXTREMELY IMPORTANT
...
C  Prepares Linearization of Exact Direct Beam reflection
...
C  Prepares Linearization of Exact Direct Beam reflection
C   Linearization with respect to all surface parameters
...
C  LAMBERTIAN: Only 1 Stokes component, only one Surface weighting function
...
C  Get the source function linearization
          ...
          DO UA = 1, N_USER_RELAZMS
            ...
            IF ( DO_REFLECTED_DIRECTBEAM(IB) ) THEN
              ...
              DO Q = 1, NSURFWFS   
                DO O1 = 1, NELEMENTS
                  IF ( DO_LAMBERTIAN_SURFACE ) THEN
                    LS_EXACTDB_SOURCE(Q,V,O1) =
     &                     REFL_ATTN * LAMBERTIAN_ALBEDO
                  ELSE                    
                    SUM = ZERO
                    DO O2 = 1, NSTOKES
