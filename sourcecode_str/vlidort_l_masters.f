...
C #                                                             #
C #            VLIDORT_L_MASTER (master)                        #
C #            VLIDORT_L_FOURIER (master)                       #
C #                                                             #
C ###############################################################
...
C  Miscellaneous and layer input.
      ...
      IF ( STATUS_SUB .EQ. VLIDORT_WARNING ) THEN
        STATUS_INPUTCHECK = VLIDORT_WARNING
        TRACE = 'Derive_Input Call in VLIDORT_L_MASTER'
        CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS_INPUTCHECK )
      ENDIF
...
C  error handling
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          MAIL = 'Error from VLIDORT_L_FOURIER, Fourier '//CF
          TRACE = 'Called by VLIDORT_L_MASTER'
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS_CALCULATION )
          CLOSE(VLIDORT_ERRUNIT)
          RETURN
        ENDIF
...
C       = 1 / 4.pi with beam sources,  = 1 for Thermal alone.    ! OLD
C       = 1                                                      ! NEW
...
C  Reflected Direct beam attenuation + DB correction (if flagged)
C  Full single scatter calculation, return after completion

          IF ( DO_SSFULL ) THEN
            CALL VLIDORT_DBCORRECTION ( SS_FLUX_MULTIPLIER )
            IF ( DO_PROFILE_LINEARIZATION ) THEN
              CALL VLIDORT_LAP_DBCORRECTION (SS_FLUX_MULTIPLIER)
            ENDIF
            IF ( DO_COLUMN_LINEARIZATION ) THEN
              CALL VLIDORT_LAC_DBCORRECTION (SS_FLUX_MULTIPLIER)
            ENDIF
            IF ( DO_SURFACE_LINEARIZATION ) THEN
              CALL VLIDORT_LS_DBCORRECTION (SS_FLUX_MULTIPLIER)
            ENDIF
            RETURN
          ELSE
...
C  End fourier = 0, user stream  clauses

        ENDIF
       ENDIF

C  Surface direct beam
...
C  .. error tracing
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #1 in VLIDORT_L_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  .. error tracing
          ...
          IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
            ...
            TRACE= 'Call #2 in VLIDORT_L_FOURIER'
            STATUS = VLIDORT_SERIOUS
            CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
            RETURN
          ENDIF
...
C  standard case using compression of band matrices, etc..
     ...
     O       STATUS_SUB )
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #3 in VLIDORT_L_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  Telescoped case
     ...
     O       STATUS_SUB )
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #4 in VLIDORT_L_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  With linearization
       ...
       IF ( DO_ATMOS_LINEARIZATION ) THEN
         ...
         IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
           TRACE  = 'Call #4A in VLIDORT_L_FOURIER'
           STATUS = VLIDORT_SERIOUS
           CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
           RETURN
         ENDIF
...
C  No linearization
       ...
       ELSE
         ...
         IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
           TRACE  = 'Call #4B in VLIDORT_L_FOURIER'
           STATUS = VLIDORT_SERIOUS
           CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
           RETURN
         ENDIF
...
C  Error handling
      ...
      IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
        ...
        TRACE= 'Call #5A in VLIDORT_L_FOURIER'
        STATUS = VLIDORT_SERIOUS
        CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
        RETURN
      ENDIF
...
C  Get the BVP solution (regular case) for this beam component
     ...
     O         STATUS_SUB )
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
               ...
               TRACE= 'Call #5B in VLIDORT_L_FOURIER'
               STATUS = VLIDORT_SERIOUS
               CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
               RETURN
              ENDIF
...
C  Get the linearized BVP solution for this beam component
     ...
     O         STATUS_SUB )
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #5C in VLIDORT_L_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  Only do this if there is surface reflectance !
     ...
     I              DO_INCLUDE_SURFEMISS,
     I              DO_INCLUDE_MVOUTPUT,
     I              N_SURFACE_WFS, FOURIER_COMPONENT, IPARTIC,
     I              SURFACE_FACTOR, FLUX_MULTIPLIER,
     O              STATUS_SUB )

          IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
            MAIL = 'Error in module VLIDORT_SURFACE_WFS'
            TRACE= 'Call #5D in VLIDORT_L_FOURIER'
            STATUS = VLIDORT_SERIOUS
            CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
            RETURN
          ENDIF
...
C  end of surface weighting functions
...
C  For the particular integral itself
     ...
     I         ( LAYER, FOURIER_COMPONENT, IBEAM, STATUS_SUB )
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                ...
                TRACE= 'Call #6A in VLIDORT_L_FOURIER'
                STATUS = VLIDORT_SERIOUS
                CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
                RETURN
              ENDIF
...
C  For the linearizations of the classical solution
     ...
     &             'Error return from module VLIDORT_L_QBEAM_SOLUTION'
                  TRACE= 'Call #6B in VLIDORT_L_FOURIER'
                  STATUS = VLIDORT_SERIOUS
                  CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
                  RETURN
                ENDIF
...
C  Get the BVP solution (regular case) for this beam component
     ...
     O         STATUS_SUB )
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              MAIL = 'Error return from module BVP_SOLUTION_MASTER'
              TRACE= 'Call #7A in VLIDORT_L_FOURIER'
              STATUS = VLIDORT_SERIOUS
              CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
              RETURN
            ENDIF
...
C  Telescoped case
     ...
     O           STATUS_SUB )
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              ...
              TRACE= 'Call #7B in VLIDORT_L_FOURIER'
              STATUS = VLIDORT_SERIOUS
              CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
              RETURN
            ENDIF
...
C    (a) Regular case (full matrix linearization)
     ...
     O         STATUS_SUB )
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
               ...
               TRACE= 'Call #8A in VLIDORT_L_FOURIER'
               STATUS = VLIDORT_SERIOUS
               CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
               RETURN
              ENDIF
...
C    (b) telescoped case 
     ...
     O         STATUS_SUB )
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
               ...
               TRACE= 'Call #8B in VLIDORT_L_FOURIER'
               STATUS = VLIDORT_SERIOUS
               CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
               RETURN
              ENDIF
...
C  Get the linearized BVP solution for this beam component
     ...
     O         STATUS_SUB )
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
               MAIL = 'Error in module L_BVP_SOLUTION_MASTER'
               TRACE= 'Call #9A in VLIDORT_L_FOURIER'
               STATUS = VLIDORT_SERIOUS
               CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
               RETURN
              ENDIF
...
C  (b) telescoped BVP linearization
     ...
     O         STATUS_SUB )
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
               MAIL = 'Error in module L_BVPTEL_SOLUTION_MASTER'
               TRACE= 'Call #9B in VLIDORT_L_FOURIER'
               STATUS = VLIDORT_SERIOUS
               CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
               RETURN
              ENDIF
...
C  Only do this if there is surface reflectance !
     ...
     O              STATUS_SUB )
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                MAIL = 'Error in module VLIDORT_SURFACE_WFS'
                TRACE= 'Call #10 in VLIDORT_L_FOURIER'
                STATUS = VLIDORT_SERIOUS
                CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
                RETURN
              ENDIF
