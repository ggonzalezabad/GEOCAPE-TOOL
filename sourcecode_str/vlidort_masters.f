...
C #                                                             #
C #            VLIDORT_MASTER (master)                          #
C #            VLIDORT_FOURIER (master)                         #
C #                                                             #
C ###############################################################
...
C  -----------
      ...
      IF ( STATUS_SUB .EQ. VLIDORT_SERIOUS ) THEN
        ...
        TRACE = ' Called in VLIDORT_MASTER '
...
C  Miscellaneous and layer input.
      ...
      IF ( STATUS_SUB .EQ. VLIDORT_WARNING ) THEN
        STATUS_INPUTCHECK = VLIDORT_WARNING
        TRACE = 'Derive_Input Call in VLIDORT_MASTER'
        CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS_INPUTCHECK )
      ENDIF
...
C  error handling
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          MAIL  = 
     &  'Error from VLIDORT_FOURIER, Fourier = '//CF
          TRACE = 'Called by VLIDORT_MASTER'
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS_CALCULATION )
          CLOSE(VLIDORT_ERRUNIT)
          RETURN
        ENDIF
...
c      write(*,*)stokes_ss(1,1,1,1),stokes_db(1,1,1)
c      write(*,*)stokes_ss(2,1,1,1),stokes_db(2,1,1)
c      write(*,*)stokes_ss(2,1,1,2),stokes_db(4,5,1)
...
C  .. error tracing
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #1 in VLIDORT_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  standard case using compression of band matrices, etc..
     ...
     O       STATUS_SUB )
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #2 in VLIDORT_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  Telescoped case
     ...
     O       STATUS_SUB )
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          TRACE= 'Call #3 in VLIDORT_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  REMARK. No Green's function treatment here
      ...
      IF ( DO_INCLUDE_THERMEMISS ) THEN
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          TRACE= 'Call #4 in VLIDORT_FOURIER'
          STATUS = VLIDORT_SERIOUS
          CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
          RETURN
        ENDIF
...
C  Error handling
      ...
      IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
        ...
        TRACE= 'Call #5A in VLIDORT_FOURIER'
        STATUS = VLIDORT_SERIOUS
        CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
        RETURN
      ENDIF
...
C    NO modulus on the thermal contribution.
C   Bug fixed 26 January 2010
...
C  Get the BVP solution (regular case) for this beam component
     ...
     O         STATUS_SUB )
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              MAIL = 'Error return from module BVP_SOLUTION_MASTER'
              TRACE= 'Call #5 in VLIDORT_FOURIER'
              STATUS = VLIDORT_SERIOUS
              CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
              RETURN
            ENDIF
...
C  Telescoped case
     ...
     O           STATUS_SUB )
            ...
            IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
              ...
              TRACE= 'Call #6 in VLIDORT_FOURIER'
              STATUS = VLIDORT_SERIOUS
              CALL VLIDORT_ERROR_TRACE ( MAIL, TRACE, STATUS )
              RETURN
            ENDIF
