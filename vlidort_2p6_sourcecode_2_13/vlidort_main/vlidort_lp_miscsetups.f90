              ...
              IF ( EMULT_HOPRULE(N,UM,IB) ) THEN
                ...
                DO Q = 1, K_PARAMETERS
                  ...
                  IF ( INITIAL_TRANS(N,IB).NE.ZERO ) &
                      V2 = V2 + LP_INITIAL_TRANS (N,K,IB,Q)
                  LP_EMULT_DN(UM,N,K,IB,Q) = EMULT_DN(UM,N,IB) * (SD+V2)
                ENDDO
              ELSE
                SD = ITRANS_USERM(N,UM,IB) / SIGMA_M(N,UM,IB)
                DO Q = 1, K_PARAMETERS
            ...
            DO UM = 1, N_USER_STREAMS
              IF ( EMULT_HOPRULE(N,UM,IB) ) THEN
                ...
                DO Q = 1, K_PARAMETERS
                  SD = V1 * L_DELTAU_VERT(Q,N) + &
                       V2 * LP_AVERAGE_SECANT(N,K,IB,Q)
                  LP_UT_EMULT_DN(UM,UT,K,IB,Q) = &
                     UT_EMULT_DN(UM,UT,IB) * SD
                ENDDO
              ELSE
                SD = ITRANS_USERM(N,UM,IB) / SIGMA_M(N,UM,IB)
                DO Q = 1, K_PARAMETERS
          ...
          IF ( K.EQ.N ) THEN
            IF ( EMULT_HOPRULE(N,LUM,IB) ) THEN
              ...
              DO Q = 1, K_PARAMETERS
                ...
                LP_EMULT_DN(LUM,N,K,IB,Q) = EMULT_DN(LUM,N,IB) * SD
              ENDDO
            ELSE
...
!  Case (a)
          ...
          IF ( K.EQ.N ) THEN
            IF ( EMULT_HOPRULE(N,LUM,IB) ) THEN
              ...
              DO Q = 1, K_PARAMETERS
                SD = V1 * L_DELTAU_VERT(Q,N)
!mick note 1/5/2013: LIDORT does not have the following two lines
                V2 = ZERO
                IF ( INITIAL_TRANS(N,IB).NE.ZERO ) &
                    V2 = V2 + LP_INITIAL_TRANS(N,K,IB,Q)
                LP_EMULT_DN(LUM,N,K,IB,Q) = EMULT_DN(LUM,N,IB) * (SD+V2)
              ENDDO
            ELSE
              SD = ITRANS_USERM(N,LUM,IB) / SIGMA_M(N,LUM,IB)
              DO Q = 1, K_PARAMETERS
...
!  Case(a)
          ...
          IF ( K.EQ.N ) THEN
            IF ( EMULT_HOPRULE(N,LUM,IB) ) THEN
              ...
              DO Q = 1, K_PARAMETERS
                SD = V1 * L_DELTAU_VERT(Q,N) + V2 * LP_AVERAGE_SECANT(N,K,IB,Q)
                LP_UT_EMULT_DN(LUM,UT,K,IB,Q) = UT_EMULT_DN(LUM,UT,IB) * SD
              ENDDO
            ELSE
              SD = ITRANS_USERM(N,LUM,IB) / SIGMA_M(N,LUM,IB)
              DO Q = 1, K_PARAMETERS
