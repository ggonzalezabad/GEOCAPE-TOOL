...
! #              VLIDORT_IQGF_UPUSER_INTENSITY (master)         #  @@@@
...
!   ADd Greens function routines

      SUBROUTINE VLIDORT_IQGF_UPUSER_INTENSITY ( &
        ...
        USER_STREAMS, KEIGEN, DELTAUS,                          & ! New
        CUMTRANS, &
...
!  Source function integrated Green function multipliers (whole layer)
      ...
      DOUBLE PRECISION, INTENT (INOUT) :: PMULT_UD &
          (MAXSTREAMS_2,MAXLAYERS,MAX_USER_STREAMS)
      DOUBLE PRECISION, INTENT (INOUT) :: PMULT_UU &
          (MAXSTREAMS_2,MAXLAYERS,MAX_USER_STREAMS)
        ...
        IF ( DO_USER_STREAMS ) THEN
          ...
          DO N = NSTART, NUT, -1
            ...
            IF ( DO_LOCAL_GREENSFUNC(N,IBEAM) ) THEN
              CALL IQGF_WHOLELAYER_STERM_UP ( &
                N, FOURIER_COMPONENT, IBEAM, SFLAG, &
                DO_INCLUDE_THERMEMISS, &
                DO_SOLAR_SOURCES, NSTOKES, NSTREAMS, &
                DO_THERMAL_TRANSONLY, DO_MSMODE_VLIDORT, &
                DO_OBSERVATION_GEOMETRY, &
                N_USER_STREAMS, LOCAL_UM_START, &
                USER_STREAMS, T_DELT_USERM, DELTAUS,     &
                INITIAL_TRANS, LAYER_PIS_CUTOFF, T_DELT_MUBAR,   &
                K_REAL, KEIGEN, T_DELT_EIGEN, LCON, MCON, &
                UHOM_UPDN, UHOM_UPUP, UPAR_UP_1, &
                ATERM_SAVE, BTERM_SAVE, GAMMA_M, GAMMA_P,  &
                HMULT_1, HMULT_2, EMULT_UP, LAYER_TSUP_UP, &
                PMULT_UD, PMULT_UU, LAYER_SOURCE )
            ELSE
...
!  Finish
      ...
      RETURN
      END SUBROUTINE VLIDORT_IQGF_UPUSER_INTENSITY
...
!  Finish
      ...
      SUBROUTINE IQGF_WHOLELAYER_STERM_UP ( &
        N, M, IBEAM, SOURCETERM_FLAG, &
        DO_INCLUDE_THERMEMISS, &
        DO_SOLAR_SOURCES, NSTOKES, NSTREAMS, &
        DO_THERMAL_TRANSONLY, DO_MSMODE_VLIDORT, &
        DO_OBSERVATION_GEOMETRY, &
...
!  real eigenvalue counter, Boundary problem constants

      INTEGER, INTENT (IN) ::           K_REAL ( MAXLAYERS )
      DOUBLE PRECISION, INTENT (IN) ::  LCON ( MAXSTRMSTKS, MAXLAYERS )
      DOUBLE PRECISION, INTENT (IN) ::  MCON ( MAXSTRMSTKS, MAXLAYERS )

!  User streams and transmittances

      DOUBLE PRECISION, intent(in)  :: T_DELT_USERM ( MAXLAYERS, MAX_USER_STREAMS )
      DOUBLE PRECISION, intent(in)  :: USER_STREAMS ( MAX_USER_STREAMS )

!  Eigenvalues and eigentransmittances defined 
...
!  Help
      ...
      LOGICAL ::          L1
      INTEGER ::          UM, O1, K, M1, AA, IB, LUM
      DOUBLE PRECISION :: H_R, SFOR1, TM, LUXR, MUXR, UDEL, SECMUM, RHOM
      DOUBLE PRECISION :: SPAR, WDEL, CONS, CONSWDEL, SD, SU, TD, TU
...
!  Offset (Complex eigenvalues only)

!      KO1 = K_REAL(N) + 1

!  Whole layer source function Homogeneous only
!    No Complex solutions here !!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  No particular solution beyond the cutoff layer.
      ...
      IF ( N .GT. LAYER_PIS_CUTOFF(IBEAM) ) RETURN
          ...
          DO K = 1, K_REAL(N)
            IF ( AAMASK(K) ) then
              ...
              IF ( ABS(RHOM ) .LT. 1.0d-03 ) THEN
                 call limit_ppgmult_wu ( rhom, secmum, cons, &
                  keigen(k,n), deltaus(n), t_delt_eigen(k,n), udel, sd)
              else
                TD = CONS     * HMULT_2(K,UM,N)
                SD = GAMMA_M(AA,N) * ( TD - EMULT_UP(UM,N,IB) )
              endif
              PMULT_UD(AA,N,UM) = SD * ATERM_SAVE(AA,N)
              TU = CONSWDEL * HMULT_1(K,UM,N)
              SU  = GAMMA_P(AA,N) * ( TU + EMULT_UP(UM,N,IB) )
              PMULT_UU(AA,N,UM) = SU * BTERM_SAVE(AA,N)
            ENDIF
          ENDDO
        ENDDO
      else
        ...
        DO K = 1, K_REAL(N)
          IF ( AAMASK(K) ) then
            ...
            IF ( ABS(RHOM ) .LT. 1.0d-03 ) THEN
              ...
              TD = CONS     * HMULT_2(K,IBEAM,N)
              SD = GAMMA_M(AA,N) * ( TD - EMULT_UP(LUM,N,IB) )
            endif
            PMULT_UD(AA,N,LUM) = SD * ATERM_SAVE(AA,N)
            TU = CONSWDEL * HMULT_1(K,IBEAM,N)
            SU  = GAMMA_P(AA,N) * ( TU + EMULT_UP(LUM,N,IB) )
            PMULT_UU(AA,N,LUM) = SU * BTERM_SAVE(AA,N)
          ENDIF
        ENDDO
      endif
        ...
        DO UM = LOCAL_UM_START, N_USER_STREAMS
          DO O1 = 1, NSTOKES_IQ
            ...
            DO K = 1, K_REAL(N)
              IF ( AAMASK(K) ) then
                AA = AA + 1
                SPAR = SPAR + UHOM_UPDN(UM,O1,K,N)*PMULT_UD(AA,N,UM) &
                            + UHOM_UPUP(UM,O1,K,N)*PMULT_UU(AA,N,UM)
              ENDIF
            ENDDO
            LAYERSOURCE(UM,O1) = LAYERSOURCE(UM,O1) + SPAR
          ENDDO
        ENDDO
      else
        DO O1 = 1, NSTOKES_IQ
          ...
          DO K = 1, K_REAL(N)
            IF ( AAMASK(K) ) then
              AA = AA + 1
              SPAR = SPAR + UHOM_UPDN(IB,O1,K,N)*PMULT_UD(AA,N,LUM) &
                          + UHOM_UPUP(IB,O1,K,N)*PMULT_UU(AA,N,LUM)
            ENDIF
          ENDDO
          LAYERSOURCE(IB,O1) = LAYERSOURCE(IB,O1) + SPAR
        ENDDO
      endif
...
!  Finish
      ...
      RETURN
      END SUBROUTINE IQGF_WHOLELAYER_STERM_UP
...
!  output arguments
      ...
      DOUBLE PRECISION, INTENT(OUT) :: gmult

!  local declarations

      DOUBLE PRECISION :: delta2,rp,smrp,h2,g2,zu,term0,term1,term2
