...
! ##########################################################
      ...
      SUBROUTINE IQGF_UPUSER_PROFILEWF ( &
        ...
        do_Local_Greensfunc, INITIAL_TRANS, T_DELT_MUBAR,      &  ! New
        LP_INITIAL_TRANS, LP_T_DELT_MUBAR, LP_AVERAGE_SECANT,  &  ! New
        USER_STREAMS, DELTAU_VERT, KEIGEN, L_KEIGEN,           &  ! New
        ...
        L_LAYER_TSUP_UTUP, &
        ATERM_SAVE, BTERM_SAVE, GAMMA_M, GAMMA_P, PMULT_UD, PMULT_UU, & ! New
        L_ATERM_SAVE, L_BTERM_SAVE, LP_GAMMA_M, LP_GAMMA_P,             & ! New
...
!  Local Greens function flag

      logical, INTENT (IN) :: do_Local_Greensfunc(MAXLAYERS,MAXBEAMS)
...
!  Holding arrays for the Green function solution

      DOUBLE PRECISION, INTENT (IN)   :: GAMMA_M(MAXSTREAMS_2,MAXLAYERS)
      DOUBLE PRECISION, INTENT (IN)   :: GAMMA_P(MAXSTREAMS_2,MAXLAYERS)
      DOUBLE PRECISION, INTENT (IN)   :: ATERM_SAVE(MAXSTREAMS_2,MAXLAYERS)
      DOUBLE PRECISION, INTENT (IN)   :: BTERM_SAVE(MAXSTREAMS_2,MAXLAYERS)

!  Linearized Holding arrays for the Green function solution

      DOUBLE PRECISION, INTENT (IN)   :: LP_GAMMA_M    &
        ( MAXSTREAMS_2, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN)   :: LP_GAMMA_P    &
        ( MAXSTREAMS_2, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN)   :: L_ATERM_SAVE &
        ( MAXSTREAMS_2, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (IN)   :: L_BTERM_SAVE &
        ( MAXSTREAMS_2, MAXLAYERS, MAX_ATMOSWFS )

!  Source function integrated Green function multipliers (whole layer)

      DOUBLE PRECISION, INTENT (IN)  :: PMULT_UD &
        ( MAXSTREAMS_2, MAXLAYERS, MAX_USER_STREAMS )
      DOUBLE PRECISION, INTENT (IN)  :: PMULT_UU &
        ( MAXSTREAMS_2, MAXLAYERS, MAX_USER_STREAMS )
...
!    3. Set multiple scatter source term (MSST) output if flagged
        ...
        IF ( DO_USER_STREAMS ) THEN
          ...
          DO N = NSTART, NUT, -1
            ...
            IF ( DO_LOCAL_GREENSFUNC(N,IBEAM) ) THEN
              CALL LP_IQGF_WHOLELAYER_STERM_UP ( &
                DO_OBSERVATION_GEOMETRY, &
                IB, N, SFLAG, NV, &
                NV_PARAMETERS, NSTOKES, NSTREAMS,&
                DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, &
                DO_MSMODE_VLIDORT, N_USER_STREAMS, LOCAL_UM_START,         &
                USER_STREAMS, T_DELT_USERM, DELTAU_VERT, L_DELTAU_VERT,    &
                K_REAL, KEIGEN, T_DELT_EIGEN, INITIAL_TRANS, T_DELT_MUBAR, &
                L_KEIGEN, L_T_DELT_EIGEN, LP_INITIAL_TRANS,                &
                LP_T_DELT_MUBAR, LP_AVERAGE_SECANT, L_LAYER_TSUP_UP,       &
                LCON, MCON,   UHOM_UPDN,   UHOM_UPUP,   UPAR_UP_1,         &
                NCON, PCON, L_UHOM_UPDN, L_UHOM_UPUP, L_UPAR_UP_1,         &
                  ATERM_SAVE,   BTERM_SAVE,    GAMMA_M,    GAMMA_P,        &
                L_ATERM_SAVE, L_BTERM_SAVE, LP_GAMMA_M, LP_GAMMA_P,        &
                  HMULT_1,   HMULT_2,    EMULT_UP, PMULT_UD, PMULT_UU,     &
                L_HMULT_1, L_HMULT_2, LP_EMULT_UP,                         &
                L_LAYER_SOURCE )
            ELSE
...
!  Finish
      ...
      RETURN
      END SUBROUTINE IQGF_UPUSER_PROFILEWF
...
!
      ...
      SUBROUTINE LP_IQGF_WHOLELAYER_STERM_UP ( &
        DO_OBSERVATION_GEOMETRY, &
        IBEAM, GIVEN_LAYER, SOURCETERM_FLAG,                           &
        LAYER_TO_VARY, NV_PARAMETERS, NSTOKES, NSTREAMS,               &
        DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY, &
        DO_MSMODE_VLIDORT, N_USER_STREAMS, LOCAL_UM_START,             &
        USER_STREAMS, T_DELT_USERM, DELTAUS, L_DELTAUS,                &
        K_REAL, KEIGEN, T_DELT_EIGEN, INITIAL_TRANS, T_DELT_MUBAR,     &
        L_KEIGEN, L_T_DELT_EIGEN, LP_INITIAL_TRANS,                    &
        LP_T_DELT_MUBAR, LP_AVERAGE_SECANT, L_LAYER_TSUP_UP,           &
        LCON, MCON,   UHOM_UPDN,   UHOM_UPUP,   UPAR_UP_1,             &
...
!  Optical depths (for small-number analysis)

      DOUBLE PRECISION, INTENT (IN) :: DELTAUS     ( MAXLAYERS )
      DOUBLE PRECISION, INTENT (IN) :: L_DELTAUS   ( MAX_ATMOSWFS, MAXLAYERS )
...
!  ---------------
      ...
      INTEGER ::          N, NV, UM, O1, Q, IB, K, AA, LUM, UM_START, UM_END
      DOUBLE PRECISION :: SHOM_R, H1, H2, H3, H4, TM
      DOUBLE PRECISION :: LUXR, MUXR, NUXR, PUXR, LLUXR, MLUXR
      DOUBLE PRECISION :: SPAR, WDEL, CONS, CONSWDEL, TD, TU, L_A, L_D, RHOM
      DOUBLE PRECISION :: L_WDEL, L_CONS, L_CONSWDEL, L_IT, L_TD, L_TU, UDEL, SECMUM
      INTEGER ::          NSTKS_NSTRMS_IQ,NSTOKES_IQ
      LOGICAL ::          AAMASK(MAXEVALUES)

      DOUBLE PRECISION :: AGM(MAXSTREAMS_2), L_AGM
      DOUBLE PRECISION :: BGP(MAXSTREAMS_2), L_BGP
      DOUBLE PRECISION :: PMULT_UD1(MAXSTREAMS_2,MAX_USER_STREAMS)
      DOUBLE PRECISION :: PMULT_UU1(MAXSTREAMS_2,MAX_USER_STREAMS)
      DOUBLE PRECISION :: L_PMULT_UD(MAXSTREAMS_2)
      DOUBLE PRECISION :: L_PMULT_UU(MAXSTREAMS_2)
...
!  Real homogeneous solutions (No complex here)
...
!  Some beam particulars
      ...
      CONS = INITIAL_TRANS(N,IBEAM)
      CONSWDEL = - CONS * WDEL
      DO AA = 1, 2*NSTREAMS
         AGM(AA) = ATERM_SAVE(AA,N) * GAMMA_M(AA,N)
         BGP(AA) = BTERM_SAVE(AA,N) * GAMMA_P(AA,N)
      ENDDO
...
!  Start parameter, local user angle loops

          DO Q = 1, NV_PARAMETERS
            L_WDEL = LP_T_DELT_MUBAR(N,NV,IB,Q)
            DO UM = LOCAL_UM_START, N_USER_STREAMS
              UDEL = T_DELT_USERM(N,UM) ; SECMUM = ONE / USER_STREAMS(UM)

!  Linearized Green's function multiplier
...
!  Down
                  IF ( ABS(RHOM).LT.1.0d-03 ) then
                     L_D = L_DELTAUS(Q,N) * DELTAUS(N)
                     L_A = LP_AVERAGE_SECANT(N,N,IB,Q)
                     CALL LIMIT_L_PPGMULT_WU &
                      ( rhom, secmum, cons, keigen(k,n), deltaus(n), &
                        t_delt_eigen(k,n), udel, &
                        L_A, L_T_DELT_EIGEN(K,N,Q), L_D, L_TD )
                     L_PMULT_UD(AA) = ATERM_SAVE(AA,N) * L_TD + &
                           PMULT_UD(AA,N,UM) * L_ATERM_SAVE(AA,N,Q)
                  ELSE
                     L_TD = CONS * L_HMULT_2(K,UM,N,Q)
                     L_TD = L_TD -  LP_EMULT_UP(UM,N,NV,IB,Q)
                     L_AGM = L_ATERM_SAVE(AA,N,Q) + LP_GAMMA_M(AA,N,NV,Q)
                     L_PMULT_UD(AA) = AGM(AA) * L_TD + PMULT_UD(AA,N,UM) * L_AGM
                  ENDIF

!  Up
                  L_BGP = L_BTERM_SAVE(AA,N,Q) + LP_GAMMA_P(AA,N,NV,Q)
                  L_TU = - CONS * ( L_HMULT_1(K,UM,N,Q) *   WDEL + &
                                      HMULT_1(K,UM,N)   * L_WDEL )
                  L_TU =  L_TU + LP_EMULT_UP(UM,N,NV,IB,Q)
                  L_PMULT_UU(AA) = BGP(AA) * L_TU + PMULT_UU(AA,N,UM) * L_BGP
...
!  Sum the particular solution contributions, add result to total
              ...
              DO O1 = 1, NSTOKES_IQ
                ...
                DO K = 1, K_REAL(N)
                  IF ( AAMASK(K) ) then
                    AA = AA + 1
                    H1 =   UHOM_UPDN(UM,O1,K,N)   * L_PMULT_UD(AA)
                    H2 = L_UHOM_UPDN(UM,O1,K,N,Q) *   PMULT_UD(AA,N,UM)
                    H3 =   UHOM_UPUP(UM,O1,K,N)   * L_PMULT_UU(AA)
                    H4 = L_UHOM_UPUP(UM,O1,K,N,Q) *   PMULT_UU(AA,N,UM)
...
!  Start parameter, local user angle loops
          ...
          DO Q = 1, NV_PARAMETERS
            L_WDEL = LP_T_DELT_MUBAR(N,NV,IB,Q)
            L_IT   = LP_INITIAL_TRANS(N,NV,IB,Q)
            DO UM = LOCAL_UM_START, N_USER_STREAMS
              UDEL = T_DELT_USERM(N,UM) ; SECMUM = ONE / USER_STREAMS(UM)
...
!  Linearized Green's function multiplier
              ...
              DO K = 1, K_REAL(N)
                IF ( AAMASK(K) ) then
                  AA = AA + 1 ; rhom = ONE / GAMMA_M(AA,N)
                  L_CONS     = CONS * L_IT
                  L_CONSWDEL = - CONS * ( L_IT*WDEL + L_WDEL )

!  Down
                  IF ( ABS(RHOM).LT.1.0d-03 ) then
                    L_A = LP_AVERAGE_SECANT(N,NV,IB,Q)
                    CALL LIMIT_L_PPGMULT_WU &
                      ( rhom, secmum, cons, keigen(k,n), deltaus(n), &
                        t_delt_eigen(k,n), udel, &
                        L_A, ZERO, ZERO, L_TD )
                    L_PMULT_UD(AA) = ATERM_SAVE(AA,N) * L_TD
                  ELSE
                    L_TD = L_CONS     * HMULT_2(K,UM,N)
                    L_TD = L_TD -  LP_EMULT_UP(UM,N,NV,IB,Q)
                    L_AGM = LP_GAMMA_M(AA,N,NV,Q)
                    L_PMULT_UD(AA) = AGM(AA) * L_TD + PMULT_UD(AA,N,UM) * L_AGM
                  ENDIF

 !  Up
                  L_TU = L_CONSWDEL * HMULT_1(K,UM,N)
                  L_TU =  L_TU + LP_EMULT_UP(UM,N,NV,IB,Q)
                  L_BGP = LP_GAMMA_P(AA,N,NV,Q)
                  L_PMULT_UU(AA) = BGP(AA) * L_TU + PMULT_UU(AA,N,UM) * L_BGP
...
!  Start parameter, local user angle loops
          ...
          DO Q = 1, NV_PARAMETERS
            L_WDEL = LP_T_DELT_MUBAR(N,NV,IB,Q)
            UDEL = T_DELT_USERM(N,IB) ; SECMUM = ONE / USER_STREAMS(IB)
                ...
                IF ( ABS(RHOM).LT.1.0d-03 ) then
                  ...
                  CALL LIMIT_L_PPGMULT_WU &
                      ( rhom, secmum, cons, keigen(k,n), deltaus(n), &
                        ...
                        L_A, L_T_DELT_EIGEN(K,N,Q), L_D, L_TD )
                  L_PMULT_UD(AA) = ATERM_SAVE(AA,N) * L_TD + &
                          PMULT_UD(AA,N,IB) * L_ATERM_SAVE(AA,N,Q)
                ELSE
                  L_TD = CONS * L_HMULT_2(K,IB,N,Q)
                  L_TD = L_TD -  LP_EMULT_UP(LUM,N,NV,IB,Q)
                  L_AGM = L_ATERM_SAVE(AA,N,Q) + LP_GAMMA_M(AA,N,NV,Q)
                  L_PMULT_UD(AA) = AGM(AA) * L_TD + PMULT_UD(AA,N,IB) * L_AGM
                ENDIF

!  Up
                L_BGP = L_BTERM_SAVE(AA,N,Q) + LP_GAMMA_P(AA,N,NV,Q)
                L_TU = - CONS * ( L_HMULT_1(K,IB,N,Q) *   WDEL + &
                                    HMULT_1(K,IB,N)   * L_WDEL )
                L_TU =  L_TU + LP_EMULT_UP(LUM,N,NV,IB,Q)
                L_PMULT_UU(AA) = BGP(AA) * L_TU + PMULT_UU(AA,N,IB) * L_BGP
...
!  Sum the particular solution contributions, add result to total
            ...
            DO O1 = 1, NSTOKES_IQ
              ...
              DO K = 1, K_REAL(N)
                IF ( AAMASK(K) ) then
                  AA = AA + 1
                  H1 =   UHOM_UPDN(IB,O1,K,N)   * L_PMULT_UD(AA)
                  H2 = L_UHOM_UPDN(IB,O1,K,N,Q) *   PMULT_UD(AA,N,IB)
                  H3 =   UHOM_UPUP(IB,O1,K,N)   * L_PMULT_UU(AA)
                  H4 = L_UHOM_UPUP(IB,O1,K,N,Q) *   PMULT_UU(AA,N,IB)
                  SPAR = SPAR + H1 + H2 + H3 + H4
                ENDIF
              ENDDO
              L_LAYERSOURCE(IB,O1,Q) = L_LAYERSOURCE(IB,O1,Q) + SPAR
...
!  Down
 ...
 !  Up
                L_TU = L_CONSWDEL * HMULT_1(K,IB,N)
                L_TU =  L_TU + LP_EMULT_UP(LUM,N,NV,IB,Q)
                L_BGP = LP_GAMMA_P(AA,N,NV,Q)
                L_PMULT_UU(AA) = BGP(AA) * L_TU + PMULT_UU(AA,N,IB) * L_BGP
...
!  Finish
      ...
      RETURN
      END SUBROUTINE LP_IQGF_WHOLELAYER_STERM_UP

!  ####################################################################

      SUBROUTINE LIMIT_L_PPGMULT_WU &
          ( EPS, SM, CONS, KALPHA, DELTA, ZDEL, UDEL, &
            L_LAMBDA, L_KALPHA, L_DELTA, L_GMULT )

!  Post-processing Green's function multiplier, whole layer upwelling
!    Small number expansion to second order. Linearized.

      IMPLICIT NONE

!  input arguments

      DOUBLE PRECISION, INTENT(IN) :: eps, sm, cons, kalpha, delta, zdel, udel
      DOUBLE PRECISION, INTENT(IN) :: l_lambda, l_kalpha, l_delta

!  output arguments

      DOUBLE PRECISION, INTENT(OUT) :: l_gmult

!  local declarations

      DOUBLE PRECISION :: delta1,delta2,delta3,beta,gamma,hh
      DOUBLE PRECISION :: rp,smrp,h2,zu,term0,term1,term2,term3
      DOUBLE PRECISION :: m0,m1,m2,m3,b0,b1,b2,b3,e0,e1,e2,e3
      DOUBLE PRECISION :: f0,f1,f2,f3,g0,g1,g2
      DOUBLE PRECISION :: aa0,aa1,aa2,aa3,a0,a1,a2,a3

!      DOUBLE PRECISION :: mf2, mf3

!  initialise

      l_gmult = 0.0d0

!  First get the expansion for the multiplier
      ...
      hh = smrp * zu * delta1
      term0 =  hh - h2*rp
      term1 = -rp*term0 - hh*delta1/2.0d0
      term2 = -rp*term1 + hh*delta2/6.0d0
      term3 = -rp*term2 - hh*delta3/24.0d0
      m0 = - cons * term0
      m1 = - cons * term1
      m2 = - cons * term2
      m3 = - cons * term3
...
!      mf2 = m0 + eps*m1 + eps*eps*m2
!      mf3 = m0 + eps*m1 + eps*eps*m2 + eps * eps * eps * m3
!      write(*,*)mf2,mf3
!      pause

!  Now write the expansion for EMULT
...
!  Now the numerator in the EMULT derivative

      beta = sm * ( l_lambda * delta + l_delta / rp )
      aa0 = beta
      aa1 =  - delta1 * beta       + sm * l_delta
      aa2 =  + delta2 * beta/2.0d0 - sm * l_delta * delta1
      aa3 =  - delta3 * beta/6.0d0 + sm * l_delta * delta2 / 2.0d0
      a0 = - l_lambda * e0 + zu * aa0
      a1 = - l_lambda * e1 + zu * aa1
      a2 = - l_lambda * e2 + zu * aa2
      a3 = - l_lambda * e3 + zu * aa3

!  Now the denominator in the EMULT derivative
...
!  Now the expansion for the EMULT derivative

      f0 = rp * a0 * b0
      f1 = rp * ( a1 * b0 + a0 * b1 )
      f2 = rp * ( a2 * b0 + a1 * b1 + a0 * b2 )
      f3 = rp * ( a3 * b0 + a2 * b1 + a1 * b2 + a0 * b3 )

!  Now the final answer

      gamma = l_lambda - l_kalpha
      g0 = - f1 * cons + gamma * m1
      g1 = - f2 * cons + gamma * m2
      g2 = - f3 * cons + gamma * m3
      l_gmult = g0 + eps * g1 + eps * eps * g2

!  Finish

      return
      END SUBROUTINE LIMIT_L_PPGMULT_WU
