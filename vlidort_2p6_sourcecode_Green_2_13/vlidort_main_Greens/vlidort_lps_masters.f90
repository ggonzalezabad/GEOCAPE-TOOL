...
!        write(*,*)' ..calculating fourier component',FOURIER_COMPONENT
        ...
        CALL VLIDORT_LPS_FOURIER ( FOURIER_COMPONENT, DO_OBSERVATION_GEOMETRY, &
          ...
          DO_SIMULATION_ONLY, DO_ATMOS_LINEARIZATION, DO_PROFILE_LINEARIZATION,                                           & !Input
          ...
          PROFILEWF_SS, PROFILEWF_DB, SURFACEWF_DB,                                                                       & !InOut
          MINT_PROFILEWF, MINT_PROFILEWF_DIRECT, FLUX_PROFILEWF, FLUX_PROFILEWF_DIRECT, MINT_SURFACEWF,FLUX_SURFACEWF,    & !InOut
          LTE_ATMOSWF,                                                                                                    & !InOut
...
!
      ...
      SUBROUTINE VLIDORT_LPS_FOURIER ( FOURIER_COMPONENT, DO_OBSERVATION_GEOMETRY, &
        DO_SSCORR_NADIR, DO_SSCORR_OUTGOING, DO_SSCORR_TRUNCATION, DO_SSFULL, DO_SOLAR_SOURCES, DO_PLANE_PARALLEL,         & !Input
        DO_REFRACTIVE_GEOMETRY, DO_DELTAM_SCALING, DO_SOLUTION_SAVING, DO_BVP_TELESCOPING, DO_UPWELLING, DO_DNWELLING,     & !Input
        DO_QUAD_OUTPUT, DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY, DO_TOA_CONTRIBS, DO_DIRECT_BEAM, DO_CLASSICAL_SOLUTION,        & !Input
...
!FROM VLIDORT_L_MISCSETUPS:
      ...
      DOUBLE PRECISION, SAVE :: LP_AVERAGE_SECANT &
          ( MAXLAYERS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )
      DOUBLE PRECISION, SAVE :: LP_INITIAL_TRANS &
          ( MAXLAYERS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )
      ...
      DOUBLE PRECISION, SAVE :: L_T_DISORDS_UTUP &
          ( MAXSTREAMS, MAX_USER_LEVELS, MAX_ATMOSWFS )
      DOUBLE PRECISION, SAVE :: LP_T_DELT_MUBAR &
          ( MAXLAYERS, MAXLAYERS, MAXBEAMS, MAX_ATMOSWFS )
      DOUBLE PRECISION, SAVE :: LP_T_UTDN_MUBAR &
...
!FROM THERMAL_SETUP_PLUS:
      ...
      DOUBLE PRECISION, SAVE :: L_T_UT_DIRECT_DN &
          ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAX_ATMOSWFS )
!FROM L_EMULT_MASTER:
      DOUBLE PRECISION, SAVE :: LP_EMULT_UP &
          ( MAX_USER_STREAMS, MAXLAYERS, MAXLAYERS, &
            MAXBEAMS, MAX_ATMOSWFS )
      DOUBLE PRECISION, SAVE :: LP_EMULT_DN &
          ( MAX_USER_STREAMS, MAXLAYERS, MAXLAYERS, &
            MAXBEAMS, MAX_ATMOSWFS )
      DOUBLE PRECISION, SAVE :: LP_UT_EMULT_UP &
          ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAXLAYERS, &
            MAXBEAMS, MAX_ATMOSWFS )
      DOUBLE PRECISION, SAVE :: LP_UT_EMULT_DN &
          ( MAX_USER_STREAMS, MAX_PARTLAYERS, MAXLAYERS, &
            MAXBEAMS, MAX_ATMOSWFS )
...
!  Local Greens' function stuff
!  ============================

      double precision :: ATERM_SAVE(MAXSTREAMS_2,MAXLAYERS)
      double precision :: BTERM_SAVE(MAXSTREAMS_2,MAXLAYERS)
      double precision :: GAMMA_M(MAXSTREAMS_2,MAXLAYERS)
      double precision :: GAMMA_P(MAXSTREAMS_2,MAXLAYERS)
      double precision :: DMI(MAXSTREAMS,2), DPI(MAXSTREAMS,2)
      double precision :: NORM_SAVED(MAXLAYERS,MAXSTREAMS_2)
      double precision :: GFUNC_UP(MAXSTREAMS_2,MAXLAYERS)
      double precision :: GFUNC_DN(MAXSTREAMS_2,MAXLAYERS)

      logical :: do_Local_Greensfunc(MAXLAYERS,MAXBEAMS),do_overall_greensfunc

!  Linearized Saved quantities for the Green function solution

      DOUBLE PRECISION :: L_ATERM_SAVE(MAXSTREAMS_2,MAXLAYERS,MAX_ATMOSWFS)
      DOUBLE PRECISION :: L_BTERM_SAVE(MAXSTREAMS_2,MAXLAYERS,MAX_ATMOSWFS)

      DOUBLE PRECISION :: LP_GAMMA_M    &
        ( MAXSTREAMS_2, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: LP_GAMMA_P    &
        ( MAXSTREAMS_2, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )

!  Source function integrated Green function multipliers (whole layer)

      DOUBLE PRECISION :: PMULT_UD (MAXSTREAMS_2,MAXLAYERS,MAX_USER_STREAMS)
      DOUBLE PRECISION :: PMULT_UU (MAXSTREAMS_2,MAXLAYERS,MAX_USER_STREAMS)
...
!  With linearization.
        ...
        IF ( DO_ATMOS_LINEARIZATION ) THEN
          ...
          CALL VLIDORT_LAP_MISCSETUPS ( &
            ...
            T_DELT_USERM, T_UTDN_USERM, &
            T_UTUP_USERM, INITIAL_TRANS, AVERAGE_SECANT, &
...
!  End fourier = 0 and user stream clauses
...
!  --------------------------------------------------------
          ...
          do_overall_greensfunc = .false.

          DO LAYER = 1, NLAYERS

!  Local Greens

              DO_LOCAL_GREENSFUNC(LAYER,IBEAM) = (NSTOKES.LE.3.and.&
                LAYER_MAXMOMENTS(LAYER).eq.2.and.FOURIER_COMPONENT.eq.0)

!  Tempo switch cuts out Greens function
!              DO_LOCAL_GREENSFUNC(LAYER,IBEAM) = .false.

!           write(*,*)DO_LOCAL_GREENSFUNC(LAYER,IBEAM),LAYER_MAXMOMENTS(LAYER)
          do_overall_greensfunc = do_overall_greensfunc .or.DO_LOCAL_GREENSFUNC(LAYER,IBEAM)
...
!  For the particular integral itself
              ...
              if ( DO_LOCAL_GREENSFUNC(LAYER,IBEAM) ) then
                 call VLIDORT_IQGF_SOLUTION (                     &
                    ...
                    WUPPER, WLOWER, STATUS_SUB, MESSAGE, TRACE_1 )
              else
...
!  For the linearizations of the classical solution
              ...
              IF ( DO_ATMOS_LINEARIZATION ) THEN
                ...
                if ( DO_LOCAL_GREENSFUNC(LAYER,IBEAM) ) then
                  CALL VLIDORT_L_IQGF_SOLUTION ( &
                    LAYER, FOURIER_COMPONENT, IBEAM, NSTOKES, NSTREAMS, &
                    NMOMENTS, FLUX_FACTOR, DO_RTSOL_VARY, NPARAMS_VARY, &
                    DO_LAYER_SCATTERING, LAYER_PIS_CUTOFF, DFLUX,       &
                    QUAD_STRMWTS, QUAD_WEIGHTS, L_OMEGA_GREEK,          &
                    PI_XQP, PI_X0P, PI_XQM_POST, K_REAL,                &
                    SOLA_XPOS, L_SOLA_XPOS, NORM_SAVED,                 &
                    DMI, DPI, ATERM_SAVE, BTERM_SAVE,                   & 
...
!  (classical)
                IF ( DO_CLASSICAL_SOLUTION ) THEN
                  ...
                  IF ( DO_LOCAL_GREENSFUNC(LAYER,IBEAM) ) THEN
                    CALL VLIDORT_IQGF_USERSOLUTION ( &
                      LAYER, FOURIER_COMPONENT, IBEAM, &
                      DO_UPWELLING, DO_DNWELLING, &
                      DO_OBSERVATION_GEOMETRY, &
                      NSTOKES, FLUX_FACTOR, &
                  ...
                  IF ( DO_ATMOS_LINEARIZATION ) THEN
                    IF ( DO_LOCAL_GREENSFUNC(LAYER,IBEAM) ) THEN
                      CALL VLIDORT_L_IQGF_USERSOLUTION ( &
                        LAYER, FOURIER_COMPONENT, IBEAM, &
                        DO_RTSOL_VARY, NPARAMS_VARY, &
...
!Rob fix 4/12/12 - added DO_MSMODE_VLIDORT
            ...
            if ( do_overall_greensfunc ) then
              CALL VLIDORT_IQGF_UPUSER_INTENSITY ( &
              DO_INCLUDE_SURFACE, DO_INCLUDE_SURFEMISS, &
              DO_INCLUDE_THERMEMISS, DO_INCLUDE_DIRECTBEAM, &
              ...
              PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX, &
              UTAU_LEVEL_MASK_UP, T_DELT_USERM, T_UTUP_USERM, &
              do_Local_Greensfunc, INITIAL_TRANS,                     & ! New
              LAYER_PIS_CUTOFF, T_DELT_MUBAR,                         & ! New
              USER_STREAMS, KEIGEN, DELTAU_VERT,                      & ! New
              CUMTRANS, &
              DO_QUAD_OUTPUT, NSTREAMS, DO_LAMBERTIAN_SURFACE, &
              LAMBERTIAN_ALBEDO, USER_BRDF_F, EMISSIVITY, USER_EMISSIVITY, &
              SURFBB, DO_THERMAL_TRANSONLY, DO_DBCORRECTION, QUAD_WEIGHTS, &
              ...
              HMULT_1, HMULT_2, EMULT_UP, LAYER_TSUP_UP, &
              UT_HMULT_UU, UT_HMULT_UD, UT_EMULT_UP, LAYER_TSUP_UTUP, BRDF_F, &
              ATERM_SAVE, BTERM_SAVE, GAMMA_M, GAMMA_P,          &  ! New
              STOKES_DOWNSURF, PMULT_UD, PMULT_UU,               &  ! Some New
              CUMSOURCE_UP, STOKES_F, MS_CONTRIBS_F, BOA_THTONLY_SOURCE )
            else
              CALL VLIDORT_UPUSER_INTENSITY ( &
              DO_INCLUDE_SURFACE, DO_INCLUDE_SURFEMISS, &
...
!    (a) Regular case (full matrix linearization)
             ...
             IF ( BVP_REGULAR_FLAG(FOURIER_COMPONENT) ) THEN
              ...
              if ( do_overall_greensfunc ) then
                ...
                CALL LP_IQGF_BVP_SOLUTION_MASTER ( &
                DO_INCLUDE_SURFACE, &
                DO_REFLECTED_DIRECTBEAM(IBEAM), &
                DO_INCLUDE_THERMEMISS, &
                LAYER_TO_VARY, N_LAYER_WFS, &
                ...
                DELTAU_VERT, L_DELTAU_VERT, KEIGEN, ATERM_SAVE, & ! @@@ New Input
...
!                if ( layer_to_vary.eq.7) then
!                   write(*,*)'lin',7,L_WUPPER(6,2,7,1),L_WLOWER(3,1,7,1)
!                   write(*,*)'lin',8,L_WUPPER(6,2,8,1),L_WLOWER(3,1,8,1)
!                   write(*,*)'lin',9,L_WUPPER(6,2,9,1),L_WLOWER(3,1,9,1)
!                   pause'm=0'
!                endif

                IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                  write(CF,'(I2)')FOURIER_COMPONENT
                  TRACE_2 = 'Error return from L_IQGF_BVP_SOLUTION_MASTER, '// &
