...
!   Rob Fix 3/17/15. Exception handling updated for serious error.
          ...
          IF ( DO_SSCORR_OUTGOING .AND. .NOT.DO_FO_CALC ) THEN
           ...
           IF ( DO_ATMOS_LINEARIZATION ) THEN
            ...
            IF ( FAIL ) THEN
               TRACE_2 = 'Linearized SS correction outgoing, failed in VLIDORT_LPS_MASTER'
               TRACE_3 = ' ** VLIDORT_LPS_MASTER '
               STATUS_CALCULATION = VLIDORT_SERIOUS
               VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
               VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
           ...
           ELSE
            ...
            IF ( FAIL ) THEN
               TRACE_2 = 'SS correction outgoing, failed in VLIDORT_LPS_MASTER'
               TRACE_3 = ' ** VLIDORT_LPS_MASTER '
               STATUS_CALCULATION = VLIDORT_SERIOUS
               VLIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
               VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
...
!  Rob Fix, 20 March 2015. Output generalized for Lattice or ObsGeom

            STOKES_SS(1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:)   = &
              FO_STOKES_SS(1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:)
            STOKES_DB(1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES)     = &
              FO_STOKES_DB(1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES)

            PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,&
                         1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,:) = &
